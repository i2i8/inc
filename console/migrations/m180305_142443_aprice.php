...
/**
 * Class m180305_142443_aprice
 */
class m180305_142443_aprice extends Migration
{
    const TBL_APRICE= '{{%aprice}}';
    public function safeUp()
    {
        ...
        $this->createTable(self::TBL_APRICE, [
            'id'	       => $this->primaryKey(),
            'model_amid'   => $this->integer()->notNull()->COMMENT('品牌型号'),
            'types_atid'   => $this->integer()->notNull()->COMMENT('维修类型'),
            'aprid'        => $this->integer()->Null()->COMMENT('价格标识'),
            //nowprice
        ...
        $this->addForeignKey('amid_priceidForeignKey' , self::TBL_APRICE , 'model_amid' , '{{%amodel}}' , 'amid' , 'CASCADE', 'CASCADE');
        $this->addForeignKey('atid_priceidForeignKey' , self::TBL_APRICE , 'types_atid' , '{{%atypes}}' , 'atid' , 'CASCADE', 'CASCADE');
        // 依次为：索引名称,表名，用哪些字段来创建索引值，若是多字段的话，可以都写里头，第四个true表示是否是唯一性的。
        // 最后一个参数慎用，尤其在有外键约束的情况下，只能false，否则，从表关联字段不能被重复写入，造成数据入库失败。
        $this->createIndex('apridIndex', self::TBL_APRICE, 'aprid', false);
        /**
    ...
    {
        $this->dropTable(self::TBL_APRICE);
    }
}
